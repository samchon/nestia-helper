...
export namespace NodeTransformer
{
        ...
        ): ts.Node
    {
        if (!ts.isMethodDeclaration(method))
            return method;
        else if (method.decorators === undefined || method.decorators.length === 0)
            return method;

        const signature: ts.Signature | undefined = project.checker.getSignatureFromDeclaration(method);
        const original: ts.Type | undefined = signature && project.checker.getReturnTypeOfSignature(signature);
        const escaped: ts.Type | undefined = original && get_escaped_type(project.checker, original);
        ...
        (
            ...
            method.name, method.questionToken, 
